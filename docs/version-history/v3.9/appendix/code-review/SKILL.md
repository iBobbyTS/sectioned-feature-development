...
## External Reviewer Session Contract

- The orchestrator supplies `review_id`, provider (`SOL` or `ZCODE`), stable agent identity, base/head, review kind, and scope packet. This skill does not choose or silently replace the provider.
- One review is a persistent session. Clarification, counter-evidence, and verification of repairs for its findings must return to the same reviewer/session when supported.
- A later independent review uses a new reviewer and is eligible for provider alternation. Do not confuse a repair verification with a new clean discovery round.
- Reviewer roles are read-only with respect to product code. Emit candidate findings and a durable artifact; the main agent owns admission and repair delegation.
- A result lacking stable identity, exact reviewed head, or durable artifact is `INCOMPLETE_REVIEW`, never clean.
- For external review mode, use the same finding taxonomy and causality proof as local review. Transport/runtime failure is not a code finding.
