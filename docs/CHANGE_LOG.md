...
- 产品完成／PLAN 关闭与 Audit PENDING 分开；不得完成产品后把审计待办清空；最终交付包含实际主包／ZAS 配对路径或真实障碍。
- 继续只记录实际大阶段结果，trace 可选；一次 artifact-only correction，不新增模型、产品测试、receipt/schema/ready 校验或运行脚本。
- 后续需求不吞并旧审计范围／旧 CLEAN；历史任务不因升级重开。本次不生成旧任务补救包。
- 保留 4.5.1 规划知识、七份 Agent、配套 code-review、ZAS wait 本地修改、所有历史文件及三份 dirty CSV。
