...
REQUIRED = [
    ...
    "requirements/REQUIREMENTS.md", "MODEL-TASK-AUDIT.md", "PARALLEL-AUDIT.md", "SUBSECTION-AUDIT.md", "ADVISOR-AUDIT.md",
]
...
class AuditFinalizeTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.pack/'PROCESS-IDENTITY.json').write_text(json.dumps({'kind':'sectioned-development-process-audit','producer':'sectioned-feature-development','feature_id':'demo','run_id':'run1','feature_base':self.head,'source_head':self.head}))
    ...
    def test_foreign_audit_kind_is_rejected(self):
        f=self.pack/'PROCESS-IDENTITY.json';x=json.loads(f.read_text());x['kind']='runtime-conformance';f.write_text(json.dumps(x))
        payload=json.loads(self.run_cli('check',*self.common(),expect=1).stdout)
        self.assertTrue(any('PROCESS' in e for e in payload['errors']))

    def test_stale_identity_head_is_rejected(self):
        f=self.pack/'PROCESS-IDENTITY.json';x=json.loads(f.read_text());x['source_head']='0'*40;f.write_text(json.dumps(x))
        payload=json.loads(self.run_cli('check',*self.common(),expect=1).stdout)
        self.assertIn('PROCESS_IDENTITY_GIT_MISMATCH',payload['errors'])
