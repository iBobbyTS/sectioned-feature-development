# Current ZCode MCP Adapter

Use capability discovery at feature start. Map the installed MCP's existing high-level spawn/list/status/result/continue operations to the external-reviewer protocol; never expose or call ZCode's internal bash/edit tools as Codex MCP tools.

For review, spawn a dedicated ZCode agent with a strict read-only review packet and required Markdown/JSON artifact. Persist the returned task/session identifier as `agent_id`. If the current MCP can continue the same agent, use it for clarification and repair verification. If it cannot guarantee continuation, do not claim same-reviewer verification: ZCode may serve only as an independent second PLAN/final review, and the orchestrator must record `CONTINUATION_UNSUPPORTED`.

Before use, call available list/status/health operations and record whether the current implementation supports stable identity, cancellation, timeout, continuation, structured results, and error differentiation. Missing capability does not authorize inventing a tool name.
