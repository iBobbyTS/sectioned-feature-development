...
For DELEGATED_PASS use [delegated-pass.md](delegated-pass.md) instead; the parent owns admission and acceptance.
...
- Read [references/review-coverage.md](review-coverage.md) when selecting review lenses or inspecting a non-trivial change.
- Read [references/ai-agent-risk-catalog.md](ai-agent-risk-catalog.md) when the change was produced or materially modified by an AI agent, or changes agent tooling, prompts, permissions, CI, MCP, connectors, or generated artifacts.
- Read [references/review-loop-protocol.md](review-loop-protocol.md) for repair-enabled reviews, multi-round reviews, or orchestration.
- Read [references/ledger-templates.md](ledger-templates.md) for large reviews or any review expected to survive context compression.
- Read [references/section-review-protocol.md](section-review-protocol.md) for `SECTION`, section-scoped `DELTA`, integration-checkpoint, or final `INTEGRATION` reviews.
