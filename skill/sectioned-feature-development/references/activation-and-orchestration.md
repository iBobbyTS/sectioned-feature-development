...
8. [Candidate barrier and independent parents](#candidate-barrier-and-independent-parents)
...
Record stable task/session IDs in `FEATURE-STATE.md` and the audit trace. A generic profile label such as `root`, [@code_reviewer](subagent://code_reviewer), or `reviewer` is not sufficient identity.
...
## Candidate barrier and independent parents

The v3.9 barrier applies to each parent and frozen review candidate. PLAN approval precedes all product writes; a dependent parent starts only after its producer is accepted and integrated. Same-parent children are serial, and no writer modifies its active review snapshot.

Independent parents may overlap only under `parallel-execution.md`: distinct canonical worktrees/branches, no read/write or contract collision, no shared mutable tests/resources, explicit PLAN eligibility, and available slots. Different paths alone do not prove independence.

Persist each active parent, writer/reviewer actual ID, workspace, stage, base/head, frozen product fingerprint and next allowed phase in STATE; render FEATURE-STATE. Use `execution_artifacts.py ready`, real tool dispatch receipts, and stage-start/stage-finish records. Plan author/reviewer separation, main not coding, and exact input/handoff evidence remain mandatory. `SUCCEEDED` is not review acceptance.
