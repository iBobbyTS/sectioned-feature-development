...
The user installs the updated ZAS implementation before using this Skill. Require all ten tools: `zcode_subagent_status`, `zcode_subagent_spawn`, `zcode_subagent_wait`, `zcode_subagent_list`, `zcode_subagent_send`, `zcode_subagent_respond`, `zcode_subagent_cancel`, `zcode_subagent_result`, `zcode_subagent_close`, and `zcode_subagent_observe`.
...
4. Wait using returned next_revision, honoring wait_time. Reasoning/text revisions can arrive immediately, so do not busy-poll or rewrite the same snapshots. The host may use bounded waits; elapsed time does not authorize an observe call by itself. Wait does not read a durable observation timeline.
