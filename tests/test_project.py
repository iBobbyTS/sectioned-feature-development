...
class ProjectTests(unittest.TestCase):
 ...
 def test_exact_advisor_contract_hash(self):
  import hashlib,json
  spec=json.loads((R/'docs/version-history/v4.0/ADVISOR_CONTRACT_PRESERVATION.json').read_text())
 ...
 def test_active_doc_links(self):
  paths=[R/'README.md',R/'docs/AGENTS.example.md',*(R/'skill').rglob('*.md'),*(R/'docs/version-history/v4.0').glob('*.md')]
  for p in paths:
   for link in re.findall(r'\[[^\]]*\]\(([^)]+)\)',p.read_text()):
    if link.startswith(('http','/','~','#')) or '<' in link:continue
 ...
 def test_install_dry_run_and_apply(self):
  with tempfile.TemporaryDirectory() as t:
   ...
   self.assertTrue((home/'agents/advisor.toml').exists());self.assertTrue((home/'skills/sectioned-feature-development/SKILL.md').exists())
